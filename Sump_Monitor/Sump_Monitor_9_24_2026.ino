////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//
//    Version 6.0       Added Alexa annoucement with stop command for alert 04/13/2026 @23:30 EDT
//
//                       Alert cooldown, counter, All Clear, boot fix    04/09/2026
//
//                       Adding  HTTP over TLS (HTTPS)
//
//                       Internet Sump Pit Monitor, Datalogger and Dynamic Web Server   
//
...
//
//
//                      Note:  Uses ESP32-S3 by Espressif Community version 3.3.10"
//
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
#define online 21  //pin for online LED indicator
...
void setup(void) {
  ...
  server.on("/sump", HTTP_GET, [](AsyncWebServerRequest *request) {
    PATH = "/sump";
    accessLog();
    ipREMOTE = request->client()->remoteIP();
    if (!flag == 1) {
  ...
  server.on("/SdBrowse", HTTP_GET, [](AsyncWebServerRequest *request) {
    PATH = "/SdBrowse";
    accessLog();
    if (!flag == 1) {
  ...
  server.on("/clear-latch", HTTP_POST,
                 [](AsyncWebServerRequest *request) {
                   alertLatched = false;
...
void loop() {
  ...
  if (MINUTE % 5 == 0 && !didFire) {
    ...
    // REMOVE from ultrasonic():
    distanceToTarget = random(0, 101) / 10.0;  // ← DELETE from here
    // Already moved to 5 min conditional block ✅
...
void masterAlexa() {
  ...
  if (distanceToTarget == 0.0) {
    ...
    Serial.println("masterAlexa: FLOODING → Alexa " + dtStamp);

    // -----------------------------------------------
    // REPEATING FLOOD ALARM -- enable after evaluation
    // Repeats every 10 minutes until water clears
    // -----------------------------------------------
    /*
    if ((MINUTE % 10 == 0) && (SECOND < pass)) {
      myFlooding.sendContactEvent(true);
      Serial.println("masterAlexa: FLOOD repeat " + dtStamp);
    }
    */
  }
  ...
  // -----------------------------------------------
    ...
    myAllClear.sendContactEvent(false);
    Serial.println("masterAlexa: HIGH WATER → Alexa " + dtStamp);

    // -----------------------------------------------
    // REPEATING HIGH WATER ALARM -- enable after evaluation
    // WARNING -- could cause interruptions if water
    // stays high -- evaluate before enabling!
    // Repeats every 5 minutes until water clears
    // -----------------------------------------------
    /*
    if ((MINUTE % 5 == 0) && (SECOND < pass)) {
      myHighWater.sendContactEvent(true);
      Serial.println("masterAlexa: HIGH WATER repeat " + dtStamp);
    }
    */
  }
  ...
  // -----------------------------------------------
    ...
    if (alertFlag || floodFlag) {
      myAllClear.sendContactEvent(true);
      myHighWater.sendContactEvent(false);
      myFlooding.sendContactEvent(false);
      Serial.println("masterAlexa: ALL CLEAR → Alexa " + dtStamp);
    }
  }
}
...
void ultrasonic() {
  ...
  Serial.println(" in.");
       ...
       distanceToTarget = random(0, 101) / 10.0;  // false = bench testing
  }
