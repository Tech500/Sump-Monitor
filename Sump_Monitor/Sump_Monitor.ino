...
/////////////////////////////////////////////////////////////////////////////////////////
// 
// Version 11.0 05/13/2026  @2157 EDT
//   Fixed logic for masterAlexa function
//
// Version 10.0 05/04/2026 @ 21:30 EDT
//
// Added caddy docker container; needed for Grafana.
...
// Add to globals
bool highWaterAlerted = false;
bool allClearAlerted  = false;
...
void sendToGrafana(const char *eventType) {
  ...
  HTTPClient http;
  http.begin("http://" + String(SERVER_IP) + ":" + String(SERVER_PORT) + "/event");
...
void setup(void) {
  ...
  while (!SinricPro.isConnected()) {
    watchdogCounter = 0;  //feed the dog
    SinricPro.handle();
    delay(100);
  }
  ...
  serverAsync.on("/favicon.ico", AsyncWebRequestMethod::HTTP_GET, [](AsyncWebServerRequest *request) {
    PATH = "/FAVICON";
    if (!flag == 1) request->send(SPIFFS, "/favicon.png", "image/png");
  });

  serverAsync.on("/", AsyncWebRequestMethod::HTTP_GET, [](AsyncWebServerRequest *request) {
    PATH = "/";
    accessLog();
  ...
  serverAsync.on("/sump", AsyncWebRequestMethod::HTTP_GET, [](AsyncWebServerRequest *request) {
    PATH = "/sump";
  ...
  serverAsync.on("/SdBrowse", AsyncWebRequestMethod::HTTP_GET, [](AsyncWebServerRequest *request) {
    PATH = "/SdBrowse";
    accessLog();
  ...
  serverAsync.on("/grafana", AsyncWebRequestMethod::HTTP_GET, [](AsyncWebServerRequest *request) {
    String panel = request->getParam("panelId")->value();
    String url = "http://192.168.12.122:3000/d-solo/adqlkqm/sump-monitor?orgId=1&from=now-6h&to=now&timezone=browser&panelId=" + panel;
    request->redirect(url);
});

  serverAsync.on("/graph", AsyncWebRequestMethod::HTTP_GET, [](AsyncWebServerRequest *request) {
    PATH = "/graph";
    accessLog();
  ...
  serverAsync.on("/Show", AsyncWebRequestMethod::HTTP_GET, [](AsyncWebServerRequest *request) {
    if (!flag == 1) request->send_P(200, PSTR("text/html"), HTML4, processor4);
  });

  serverAsync.on("/get-file", AsyncWebRequestMethod::HTTP_GET, [](AsyncWebServerRequest *request) {
  ...
  serverAsync.on("/redirect/internal", AsyncWebRequestMethod::HTTP_GET, [](AsyncWebServerRequest *request) {
  ...
  // Force an immediate first reading so snapshots are valid at first page load
  readSensor();
  updateCondition();
...
void updateFiveMinuteCycle() {
    ...
    updatePumpRuntime();
    distanceLog();
    logtoSD();              // ← every 5-minute cycle, always logs
    masterAlexa(condition); // only fires when condition changed
...
void masterAlexa(String condition) {
    ...
    //Prevent redundant execution if condition hasn't changed
    if (condition == prevCondition) return;

    Serial.println("masterAlexa: condition=" + condition + " prevCondition=" + prevCondition);

    //Handle Grafana (Expects lowercase)
    // Create a local copy to avoid changing the original 'condition' string
    String grafanaCondition = condition;
    grafanaCondition.toLowerCase();
    sendToGrafana(grafanaCondition.c_str());

    //Handle Alexa / SinricPro (Expects uppercase logic)
    SinricProContactsensor &myFlooding = SinricPro[FLOODING];

    if (condition == "FLOODING") {
        myFlooding.sendContactEvent(true); 
        sendFlood(); // Triggers Claxon3 via MyMedia [cite: 138]
        Serial.println("masterAlexa: FLOODING Triggered");
    } 
    
    if (condition == "HIGHWATER") {
        sendAlert();
        Serial.println("masterAlexa: HIGHWATER email sent");
    } 
    
    if(condition == "ALLCLEAR") {
        sendAllClear();
        Serial.println("masterAlexa: ALL CLEAR Triggered");
    }
    
    if (condition == "PUMPOFF") {
      Serial.println("masterAlexa: PUMPOFF logged");
    }
...
void newDay() {
    ...
    // Archive and purge on Saturday night
    if (DOW == 6) {
        fileStore();       // rename LOG.TXT to dated name
        deleteAllFiles();  // purge everything except SYSTEM/ and MASTER.TXT
    }
...
String updateCondition() {
  ...
  else if (distanceToTarget > 0.5 && distanceToTarget <= 3.0) {  //Added condition <= 3.0 wml  --stop allclert from sending flood alert
    condition = "HIGHWATER";
    alertFlag = true;
    floodFlag = false;
  }
